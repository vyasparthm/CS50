...
def main():
    ...
    while True:
        ...
        date_by_user = input('Enter Date: ').title().strip()
        result = print_date(date_by_user,months_dict)
        if result:
            print(result)
            break
        else:
            print('Invalid Date detected, Try again!!')
...
def print_date(user_input,dict_months):
    ...
    if '/' in user_input:
         ...
         if len(parts) != 3:
             return None
         month = parts[0]
         if month in dict_months.values():
            day =  parts[1]
        ...
        parts = user_input.split()
        if not dict_months.get(parts[0]):
            print('Incorrect Month, Try Again!!')

        elif  dict_months.get(parts[0]):        
            day = parts[1].replace(',','')
            month =  dict_months.get(parts[0])
            day = int(day)
